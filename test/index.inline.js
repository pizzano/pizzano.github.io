...
const cartBottomBar = document.querySelector("#cartBottomBar");
const cartBottomBarCount = document.querySelector("#cartBottomBarCount");
const cartBottomBarTotal = document.querySelector("#cartBottomBarTotal");
...
const clearCart = document.querySelector("#clearCart") || { hidden: true, addEventListener: () => {} };
...
function syncCartBottomBarState() {
  if (!cartBottomBar) return;
  const hasCartItems = cart.length > 0;
  const shouldShow = hasCartItems && isMenuSurfaceActive();
  cartBottomBar.hidden = !shouldShow;
  cartBottomBar.setAttribute("aria-hidden", shouldShow ? "false" : "true");
  document.body.classList.toggle("cart-bottom-visible", shouldShow);
}
...
function updateTopMenuReturnButton() {
  ...
  document.body.classList.toggle("kol-top-back-active", popupOpen);
  if (siteNameText) {
    const currentName = siteNameText.querySelector(".brand-label")?.textContent?.trim() || "KØL GRILL & PIZZA";
    siteNameText.setAttribute("aria-label", popupOpen ? "Tilbake til meny" : currentName);
    siteNameText.setAttribute("aria-pressed", popupOpen ? "true" : "false");
  }
  if (brandBackLabel) brandBackLabel.setAttribute("aria-hidden", popupOpen ? "false" : "true");
}
...
function syncCategoryTabsVisibility() {
  const isCartOpen = !!(cartModal && !cartModal.hidden) || document.body.classList.contains("cart-open");
  const isInfoOpen = !!(infoModal && !infoModal.hidden) || document.body.classList.contains("info-open");
  const isProfileOpen = !!(profileModal && !profileModal.hidden) || document.body.classList.contains("profile-open");
  const isLiveOpen = !!(orderLiveModal && !orderLiveModal.hidden) ||
    (quickCheckoutModal && !quickCheckoutModal.hidden) || document.body.classList.contains("order-live-open");
  const isSettingsOpen = document.body.classList.contains("settings-open");

  // Product modal açıkken kategori barı görünebilir; info/profil/sepet/canlı siparişte gizli kalmalı.
  const shouldHide = isCartOpen || isInfoOpen || isProfileOpen || isLiveOpen || isSettingsOpen;
  document.body.classList.toggle("category-tabs-hidden", Boolean(shouldHide));
  if (categoryTabsWrap) {
    categoryTabsWrap.hidden = Boolean(shouldHide);
    categoryTabsWrap.setAttribute("aria-hidden", shouldHide ? "true" : "false");
    categoryTabsWrap.style.display = shouldHide ? "none" : "";
  }

  if (!shouldHide && categoryTabs && categoryTabs.childElementCount === 0 && menuSectionsEl?.children?.length) {
...
function syncBodyScrollLocks() {
  ...
  setPageScrollLocked(productOpen || infoOpen || cartOpen || profileOpen || liveOpen);
  syncCartBottomBarState();
...
function renderMenuRowPrice(item, rowSoldOut = false) {
  ...
  return `<strong class="row-price">${hasManyPrices ? "Fra " : ""}${formatPrice(basePrice)}</strong>`;
}
...
function playCustomerStatusSound(status = "pending") {
  // TÜRKÇE: Müşteriye kabul/iptal sesi yalnızca Grunninfo > Bestillingslyd alanına URL girilirse çalar.
  // Boşsa tarayıcı kendi kendine ses çıkarmaz.
  playCustomerCustomStatusAudio(status);
}
...
function orderReadyClockText(order = {}) {
  ...
  if (order.pickup?.mode === "later" && order.pickup?.time) {
    return `Hentetid ${formatClock(order.pickup.time)}`;
  }
  const readyAt = getCustomerReadyAt(order);
  return readyAt.getTime() <= Date.now() ? "Klar nå" : `Ca. ${formatClock(readyAt)}`;
}
...
function orderReadyDisplayParts(order = {}) {
  ...
  if (readyText === "Maten er klar") {
    return {
      ...
      value: "",
      unit: "",
      clock: "Kom og hent når det passer."
    };
  }
...
function canAcceptOrdersNow() {
  return isOrderingOpenNow();
}

function closedOrderingMessageText() {
  const pause = activeServicePause();
  if (pause) return pause.message || `Restauranten er midlertidig pauset. Bestilling kan sendes igjen kl. ${formatClock(pause.until)}.`;
  const nextOpen = nextOrderingOpenDate(new Date());
  return `Restauranten er stengt nå. Bestilling kan ikke sendes før vi åpner kl. ${formatClock(nextOpen)}.`;
}
...
function refreshHeroStatusPill() {
  const heroPill = document.querySelector(".menu-app-pill");
  if (!heroPill) return;
  const closed = !canAcceptOrdersNow();
...
function showClosedOrderingMessage() {
  ...
  if (heroCard && typeof heroCard.scrollIntoView === 'function') {
    heroCard.scrollIntoView({ block: 'start', behavior: 'smooth' });
  } else {
    alert(message);
  }
}
...
function updatePickupControls() {
  ...
  pickupHelp.textContent = openNow
    ? `Bestilling er åpen ${openLabel}–${closeLabel}. Velg hentetid fra ${formatClock(firstChoice)}, deretter hvert 15. minutt.`
    : `Restauranten er stengt nå. Du kan sende bestillingen; den behandles når vi åpner kl. ${openLabel}. Velg hentetid fra ${formatClock(firstChoice)}.`;
}
...
function buildOrderPayload() {
  ...
  return {
    ...
    items: cart.map((line) => ({
      ...
      extras: line.extras || [],
      extraDetails: line.extraDetails || [],
...
function orderStatusHtml(order = {}, options = {}) {
  ...
  const countdownText = waitingOpen
    ...
    : status === "pending"
      ...
      : "";
  const countdownLabel = waitingOpen ? "Åpner" : "Svar innen";
  ...
  return `
    <div class="order-live-status ${status} ${waitingOpen ? "waiting-open" : ""}">
      ...
      ${message ? `<p>${message}</p>` : ""}
      ${status === "pending" ? `<div class="order-countdown-box ${expired ? "expired" : ""}"><span>${countdownLabel}</span><strong data-order-countdown="${escapeAttribute(order.id || "")}">${expired ? "Litt forsinket" : countdownText}</strong></div>${expiredHelp}` : ""}
...
function applySiteSettings() {
  const settings = normalizeSiteSettings(siteSettings);
  applyGlobalTypographySettings(settings);
  ...
  if (siteNameText) {
    ...
    if (!topBrandLabel || !topBackLabel) {
      siteNameText.innerHTML = '<span class="brand-label"></span><span class="brand-back-label" aria-hidden="true">Tilbake</span>';
      topBrandLabel = siteNameText.querySelector(".brand-label");
      topBackLabel = siteNameText.querySelector(".brand-back-label");
    }
    if (topBrandLabel) topBrandLabel.textContent = name.toUpperCase();
    if (topBackLabel) {
...
function centerCategoryTab(sectionId, behavior = "smooth") {
...
function setActiveCategoryTab(sectionId, options = {}) {
  ...
  const { centerIntoView = true, behavior = "smooth" } = options;
...
function getRenderableMenuSections() {
  ...
  return mostOrderedSection ? [mostOrderedSection, ...sections] : sections;
}
...
function refreshCategoryTabsFromCurrentMenu(options = {}) {
  ...
  renderCategoryTabs(sectionsToRender);

  const nextActive = sectionsToRender.some((section) => section.id === previousActive)
    ? previousActive
    : (sectionsToRender[0]?.id || "");
...
function renderCategoryTabs(sections) {
  ...
  categoryTabsWrap.hidden = sections.length <= 1;
  syncCategoryTabsVisibility();
  syncCategoryTabsHeight();
...
function scrollToMenuSection(sectionId) {
  ...
  setActiveCategoryTab(sectionId, { centerIntoView: true, behavior: "smooth" });

  window.requestAnimationFrame(() => {
    const targetTop = window.pageYOffset + panel.getBoundingClientRect().top - getMenuStickyOffset();
...
function updateActiveCategoryFromScroll() {
  ...
  const panels = [...menuSectionsEl.querySelectorAll(".category-panel[data-section]")];
  if (!panels.length) return;

  const probeLine = getMenuStickyOffset() + 8;
  ...
  if (nextId && nextId !== activeCategoryTabId) {
    setActiveCategoryTab(nextId, { centerIntoView: true, behavior: "smooth" });
  }
}
...
function markSelectedMenuProduct(productId = "") {
  ...
  menuSectionsEl.querySelectorAll(".menu-row[data-product]").forEach((row) => {
    ...
    const side = row.querySelector(".menu-app-product-side");
    const price = row.querySelector(".menu-row-inline-price");
    ...
    if (isMarked) {
      if (!chip && side) {
        ...
        side.insertBefore(chip, price || null);
      } else if (chip) {
        chip.textContent = "Valgt";
        if (side && chip.parentElement !== side) side.insertBefore(chip, price || null);
      }
    } else if (chip) {
      chip.remove();
    }
...
function renderMenuHero() {
  return `
    <section class="menu-search-card">
      <span aria-hidden="true">⌕</span>
      <input id="menuSearchInput" type="search" autocomplete="off" inputmode="search" placeholder="Søk i menyen" value="${escapeAttribute(menuSearchQuery)}">
    </section>
  `;
}
    ...
    mount.innerHTML = `
      <section class="menu-app-hero info-hero-card">
        ...
        <div class="menu-app-hero-copy">
          <span class="menu-app-pill">${canAcceptOrdersNow() ? openStatusShortText() : closedStatusShortText()}</span>
...
function filterMenuRows(query = "") {
  const normalized = String(query || "").trim().toLowerCase();
  if (!menuSectionsEl) return;

  const heroEl = menuSectionsEl.querySelector(".menu-app-hero");
  const resultSectionId = "menuSearchResults";
  let resultSection = menuSectionsEl.querySelector(`#${resultSectionId}`);
  const normalPanels = menuSectionsEl.querySelectorAll(".category-panel[data-section]");

  // Arama yoksa sadece sonuç alanını kaldır, normal menüyü aynen göster.
  if (!normalized) {
    ...
    normalPanels.forEach((panel) => {
      panel.hidden = false;
      panel.querySelectorAll(".menu-row[data-product]").forEach((row) => {
        row.hidden = false;
      });
    });
    requestCategoryScrollSync();
  ...
  // Arama sonuçları üstte gösterilir; normal kategoriler aşağıda kalır ve kategori çubuğu çalışmaya devam eder.
  normalPanels.forEach((panel) => {
    panel.hidden = false;
    panel.querySelectorAll(".menu-row[data-product]").forEach((row) => {
      row.hidden = false;
  ...
  const rows = [...menuSectionsEl.querySelectorAll(".category-panel[data-section] .menu-row[data-product]")];
  ...
  rows.forEach((row) => {
    ...
    const existing = matches.get(productId);
    const rowIsSynthetic = row.dataset.synthetic === "true";

    // Aynı ürün hem "Mest bestilt" hem kendi kategorisinde varsa gerçek kategorideki satırı tercih et.
    if (!existing || (existing.dataset.synthetic === "true" && !rowIsSynthetic)) {
      matches.set(productId, row);
    }
  });

  const searchCard = menuSectionsEl.querySelector(".menu-search-card");
  if (!resultSection) {
    resultSection = document.createElement("section");
    resultSection.id = resultSectionId;
    resultSection.className = "category-panel menu-app-section menu-search-results-section";
    resultSection.setAttribute("data-search-results", "true");
    if (searchCard?.nextSibling) menuSectionsEl.insertBefore(resultSection, searchCard.nextSibling);
    else menuSectionsEl.appendChild(resultSection);
  }

  const matchRows = [...matches.values()];
  const titleText = "Søkeresultater";
  const countText = `${matchRows.length} varer`;
  const queryText = normalized.replace(/</g, "&lt;").replace(/>/g, "&gt;");

  if (!matchRows.length) {
    resultSection.innerHTML = `
      <div class="menu-app-section-head search-results-head">
        <div>
          <h2>${titleText}</h2>
          <p>Ingen produkter matcher “${queryText}”.</p>
        </div>
        <span>0 varer</span>
      </div>
      <div class="menu-search-empty">Ingen produkter funnet.</div>
    `;
    requestCategoryScrollSync();
    return;
  }
  ...
  resultSection.innerHTML = `
    <div class="menu-app-section-head search-results-head">
      <div>
        <h2>${titleText}</h2>
        <p>Produkter som matcher “${queryText}”.</p>
      </div>
      <span>${countText}</span>
    </div>
  `;
...
function renderMenu() {
  ...
  menuSectionsEl.innerHTML = renderMenuHero() + sectionsToRender
    .map((section, index) => {
      const sectionSoldOut = isSoldOutItem(section);
      const leadText = getMenuSectionLeadText(section);
      return `
        <section class="category-panel menu-app-section ${sectionSoldOut ? "category-sold-out" : ""}" data-section="${section.id}">
          <div class="menu-app-section-head">
            ...
            <span>${asArray(section.items).length} varer</span>
          </div>
          <div class="menu-list">
            ${section.items
              .map((item) => {
                ...
                const isSelected = !section.isSyntheticMostOrdered && getMarkedMenuProductIds().has(String(item.id));
                const searchText = `${section.title} ${prefix}${item.name} ${details}`.toLowerCase();
                return `
                  <button class="menu-row menu-app-product ${rowSoldOut ? "sold-out" : ""} ${isSelected ? "selected-product" : ""}" type="button" data-product="${item.id}" data-product-section="${section.id}" data-source-section="${item._sourceSectionId || section.id}" data-synthetic="${section.isSyntheticMostOrdered ? "true" : "false"}" data-search="${escapeAttribute(searchText)}" ${isSelected ? 'aria-current="true"' : ""} ${rowSoldOut ? 'disabled aria-disabled="true"' : ""}>
                    ${renderThumb(item)}
                    ...
                    <span class="menu-app-product-side">
                      <span class="menu-app-plus" aria-hidden="true">+</span>
  ...
  filterMenuRows(menuSearchQuery);
...
function renderProductModal() {
  ...
  const finalProductTitle = `${titlePrefix}${selectedProduct.name.toUpperCase()}`;
...
function cartLineThumbHtml(line = {}) {
  // V193: Sepette ürün resmi istenmiyor. Bu yardımcı fonksiyon boş döner,
  // böylece eski/farklı render yolları tekrar resim basamaz.
  return "";
}
...
function updateCartLineQuantity(index, delta) {
  ...
  const nextQuantity = Math.max(0, Number(cart[lineIndex].quantity || 1) + Number(delta || 0));
  if (nextQuantity <= 0) {
    expandedCartQtyIndex = null;
    kolRemoveCartLineDirect(lineIndex);
    return;
  }
...
function renderCart() {
  ...
  const hasCartItems = cart.length > 0;
  if (cartBottomBarCount) cartBottomBarCount.textContent = `${itemCount} ${itemCount === 1 ? "vare" : "varer"}`;
  if (cartBottomBarTotal) cartBottomBarTotal.textContent = formatPrice(cartSubtotal);
  if (cartBottomBar && !showOnlyOrderStatus) syncCartBottomBarState();
  else if (cartBottomBar) {
    cartBottomBar.hidden = true;
    cartBottomBar.setAttribute("aria-hidden", "true");
  }
  ...
  clearCart.hidden = !hasCartItems;
  checkoutButton.disabled = !hasCartItems;
  document.body.classList.toggle("cart-has-items", hasCartItems);
  document.body.classList.toggle("cart-empty-state", !hasCartItems);
  if (!showOnlyOrderStatus) syncCartBottomBarState();
  else document.body.classList.remove("cart-bottom-visible");
  ...
  cartItems.innerHTML = cart
    .map((line, index) => {
      ...
      if (hasPaidExtras && !detailRows.some((detail) => /^sum\s*:/i.test(String(detail?.label || "")))) {
        detailRows.push({ label: "Sum:", priceText: formatPrice(line.total), isLineTotal: true });
      }
      return `
        <article class="cart-item cart-item-modern cart-item-no-image" data-cart-line="${index}" role="button" tabindex="0" aria-label="Rediger ${escapeAttribute(line.name)}">
          ...
          <div class="cart-item-actions" aria-label="Rediger eller slett">
            ...
            <button class="edit-line-button icon-only" type="button" data-edit="${index}" data-cart-action="edit" aria-label="Rediger vare" title="Rediger vare"><span class="cart-edit-icon" aria-hidden="true">✎</span></button>
          </div>
        </article>
      `;
    })
...
menuSectionsEl.addEventListener("input", (event) => {
  if (event.target?.id !== "menuSearchInput") return;
...
if (false && productTitlebar) {
  productTitlebar.addEventListener("click", (event) => {
    const isButton = event.target.closest("button");
    if (isButton) return;
    closeProductModal();
  });
  productTitlebar.addEventListener("keydown", (event) => {
...
if (cartBottomBar) cartBottomBar.addEventListener("click", () => { updatePickupControls(); renderRecentOrders(); openCart(); });
...
if (backToMenu) backToMenu.addEventListener("click", closeAllCustomerPopupsAndShowMenu);
...
/* ===== V63: DIRECT cart edit fix =====
   This does NOT depend on SVG, ::before, onclick, or the icon element.
   It calls the original working openCartLineEditor(index) directly. */
let kolLastCartEditAt = 0;
...
function kolEditCartLineDirect(index) {
  ...
  if (typeof openCartLineEditor === "function") {
    openCartLineEditor(index);
    return false;
  }

  // Fallback only if old function is missing.
  const line = cart[index];
  const result = findProduct(line?.productId);
  ...
  applyDefaultOptionSelections();
  quantity = line.quantity || 1;
  if (specialInstructions) specialInstructions.value = line.note || "";
  closeCartModal();
  renderProductModal();
  productModal.hidden = false;
  document.body.classList.add("modal-open");
...
window.__kolEditCartLine = kolEditCartLineDirect;
window.__kolRemoveCartLine = kolRemoveCartLineDirect;
...
function kolCartActionFromEvent(event) {
  ...
  // Slett/antall skal KUN skje på click. Ikke på touchstart/pointerdown.
  if ((removeButton || qtyToggleButton || qtyIncButton) && event.type !== "click") return;
  if (!actionButton && event.type !== "click") return;
  ...
  if (editIndex !== undefined) {
    const now = Date.now();
    if (now - kolLastCartEditAt < 350) return;
    kolLastCartEditAt = now;
    kolEditCartLineDirect(editIndex);
  }
}

// Edit needs early capture. Remove is guarded to click only inside the handler.
document.addEventListener("pointerdown", kolCartActionFromEvent, true);
document.addEventListener("click", kolCartActionFromEvent, true);
document.addEventListener("touchstart", kolCartActionFromEvent, { capture: true, passive: false });
