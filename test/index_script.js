...
function syncCategoryTabsVisibility() {
  ...
  const isLiveOpen = !!(orderLiveModal && !orderLiveModal.hidden) || document.body.classList.contains("order-live-open");
...
function renderMenuRowPrice(item, rowSoldOut = false) {
  ...
  if (sizes.length > 1) {
    return `
      <span class="row-price-group multi" aria-label="Priser">
        ${sizes
          .map((size) => `<strong class="row-price-inline" data-size="${size.id}">${formatMenuCompactPrice(size.price)}</strong>`)
          .join('<span class="row-price-separator" aria-hidden="true">|</span>')}
      </span>
    `;
  }
  const price = item.displayPrice ?? getBasePrice(item, getDefaultProductSizeId(item));
  return `<strong class="row-price">${formatPrice(price)}</strong>`;
}
...
function applySiteSettings() {
  ...
  if (siteNameText) {
    ...
    if (!topBrandLabel || !topBackLabel) {
      siteNameText.innerHTML = '<span class="brand-label"></span><span class="brand-back-label" aria-hidden="true">← Tilbake til meny</span>';
    ...
    if (topBackLabel) {
      topBackLabel.textContent = "← Tilbake til meny";
...
function buildCartLine() {
  ...
  const sizeLabel = sizeOptions.length
    ...
    : "Størrelse: Standart";
  const extraDetails = [];
  if (sizeOptions.length && sizeLabel !== "Størrelse: Standart") {
    extraDetails.push({ label: sizeLabel, price: 0, priceText: "" });
  }
          ...
          <div class="menu-list">
            ${section.items
              .map((item) => {
                ...
                return `
                  <button class="menu-row ${rowSoldOut ? "sold-out" : ""} ${isSelected ? "selected-product" : ""}" type="button" data-product="${item.id}" ${isSelected ? 'aria-current="true"' : ""} ${rowSoldOut ? 'disabled aria-disabled="true"' : ""}>
                    ${renderThumb(item)}
                    ...
                    ${renderMenuRowPrice(item, rowSoldOut)}
                  </button>
                `;
              })
              .join("")}
      ...
      return `
        <article class="cart-item">
          <div class="cart-item-main cart-item-main-stacked">
            ...
            ` : ""}
            ${line.note ? `<p class="cart-item-note">Notat: ${line.note}</p>` : ""}
          </div>
          <div class="cart-item-actions">
            <div class="cart-item-top-actions">
            ...
            <strong class="cart-line-total">${formatPrice(line.total)}</strong>
          </div>
        </article>
      `;
    })
...
function kolCartActionFromEvent(event) {
  ...
  const removeButton = event.target.closest?.(".remove-button[data-remove], [data-cart-action='remove']");
  const button = editButton || removeButton;
  if (!button || !cartItems?.contains(button)) return;

  // Slett skal KUN skje på click. Ikke på touchstart/pointerdown.
  // Ellers mobil kan slette flere varer med én berøring.
  if (removeButton && event.type !== "click") return;
  ...
  if (editButton) {
    ...
    kolEditCartLineDirect(editButton.dataset.edit);
    return;
  }
...
document.addEventListener("keydown", (event) => {
  ...
  const editButton = event.target.closest?.(".edit-line-button[data-edit], [data-cart-action='edit']");
  if (!editButton || !cartItems?.contains(editButton)) return;
  event.preventDefault();
  kolEditCartLineDirect(editButton.dataset.edit);
}, true);
