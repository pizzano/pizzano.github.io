...
(() => {
  'use strict';
  if (window.__KOL_ADMIN_OPTIONS_BRIDGE__) return;
  window.__KOL_ADMIN_OPTIONS_BRIDGE__ = true;

  const asArray = value => Array.isArray(value) ? value : (value && typeof value === 'object' ? Object.values(value) : []);
  const baseNormalize = normalizeAdminConfigForCustomer;
  ...
  function normalizeChoiceGroup(group = {}) {
    ...
    const maxSelections = selectionMode === 'single' ? 1 : Math.max(1, Number(group.maxSelections ?? group.max) || options.length || 1);
    let defaults = asArray(group.defaultOptionIds).map(String).filter(id => options.some(option => option.id === id));
    if (selectionMode === 'single' && defaults.length > 1) defaults = defaults.slice(0, 1);
    return {
      id: String(group.id || ''),
      title: String(group.title || 'Velg'),
      selectionMode,
      required,
      maxSelections,
  ...
  normalizeAdminConfigForCustomer = function normalizeAdminConfigForCustomerWithOptions(config = {}) {
    const normalized = baseNormalize(config);
    const rawSections = asArray(config.sections);
    const allGroups = asArray(config.optionGroups).map(normalizeChoiceGroup);
    normalized.forEach(section => {
      ...
      (section.items || []).forEach(productData => {
        ...
        productData.defaultSizeIndex = Math.min(
          ...
          Math.max(0, productData.sizes.length - 1)
        );
        const ids = new Set(asArray(rawProduct.optionGroupIds).map(String));
        productData.choiceGroups = allGroups.filter(group => ids.has(group.id));
      });
    });
    return normalized;
  };
  ...
  if (window.KOLIntegration) {
    window.KOLIntegration.normalizeAdminConfigForCustomer = normalizeAdminConfigForCustomer;
  }

  function initSelections(productData) {
    const selections = {};
    (productData.choiceGroups || []).forEach(group => {
      selections[group.id] = [...group.defaultOptionIds];
      if (group.selectionMode === 'single' && selections[group.id].length > 1) selections[group.id] = selections[group.id].slice(0, 1);
      if (group.required && !selections[group.id].length && group.options.length === 1) selections[group.id] = [group.options[0].id];
    });
  ...
  function validateOptions() {
    ...
    for (const group of productData.choiceGroups || []) {
      ...
      if (group.selectionMode === 'multiple' && count > group.maxSelections) {
        showToast(`Velg maks ${group.maxSelections} i ${group.title}`);
        return false;
      }
    }
    return true;
  ...
  function ensureOptionsHost() {
    ...
    else document.querySelector('#productScreen .product-body')?.appendChild(host);
    return host;
  }
  ...
  function renderChoiceGroups() {
    ...
    host.innerHTML = groups.map(group => {
      ...
      const rule = group.selectionMode === 'multiple'
        ? `${group.required ? 'Velg minst 1' : 'Valgfritt'} · maks ${group.maxSelections}`
        : (group.required ? 'Velg 1' : 'Valgfritt');
  ...
  addOrUpdateCart = function addOrUpdateCartWithOptions() {
    ...
    if (editingIndex >= 0 && state.cart[editingIndex]) {
      state.cart[editingIndex] = entry; state.product.editingCartIndex=-1; saveStorage(STORAGE_KEYS.cart,state.cart); renderCartCount(); renderCart(); openView('cartScreen',{tabs:true}); showToast('Handlekurv oppdatert'); return;
    }
    ...
    saveStorage(STORAGE_KEYS.cart,state.cart); renderCartCount(); goMenu(); showToast(`${selected.name} lagt i handlekurven`);
  };
